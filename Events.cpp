...
//                << " and the customer arrived at " << tellers[e.tellerNum].arrivalTime << "\n\n";
    ...
    this->bankWaitTimes.push_back(totalWaitTime);
...
void Bank::simulateWaitTimes(std::priority_queue<Event> events) {
    while (!events.empty()) {
        Event e = events.top();
        if (e.isArrival) {
            // Bank Simulation
            Event tellerEvent = this->onArrival(e);
            if (!tellerEvent.isNull) {
                events.push(tellerEvent);
            }
        } else {
            Event tellerEvent = this->onTellerFinish(e);
            if (!tellerEvent.isNull) {
                events.push(tellerEvent);
            }
        }
...
//    this->storeWaitTimes.push_back(this->lineTimes[0]);
}

void Store::simulateWaitTimes(std::priority_queue<Event> events) {
    int lastEventTime = 0;
    while (!events.empty()) {
        Event e = events.top();
        int timePassed = e.timeOfEvent - lastEventTime;
        this->onArrival(e, timePassed);
